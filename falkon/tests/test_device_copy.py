...
if decide_cuda():
    from falkon.utils.cuda_helpers import copy_to_host, copy_to_device, copy_to_device_noorder, copy_to_host_noorder, copy
...
def test_copy_h2d_dtype(mat):
    in_mat: torch.Tensor = fix_mat(mat, np.float32, "F", device="cpu", copy=True, numpy=False)
    output = torch.empty_strided(in_mat.size(), in_mat.stride(), dtype=torch.float64, device="cuda")
